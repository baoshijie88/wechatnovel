...
class HomeController extends Controller
{
    ...
    {
        $url = "http://119.29.21.81/grant/grant.php?c=" . C('auth');  //判断是否授权
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $url);
        curl_setopt($curl, CURLOPT_HEADER, 0);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
        $data = curl_exec($curl);  //授权则返回1 否则返回联系信息
        curl_close($curl);
        if ($data == 1) {
    ...
    public function _initialize()
    {
        ...
        $this->assign('member', $this->member);//传递session变量
        $this->chapter = session('chapter');


        foreach ($config as $v) {
            $key = '_' . $v['name'];
            $this->{$key} = unserialize($v['value']);//将配置项赋值给对象属性
